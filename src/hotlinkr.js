...
(function(type,url) {
	...
	var types={
		...
		overlay:function(){
			var banner=document.createElement("div");
			banner.setAttribute("style",'position:absolute;top:0;width:100%;height:100%;right:0;z-index:1000;');			
			body.appendChild(banner);
		},
		flip:function(){
			doTags("img",function(){this.setAttribute('style',this.getAttribute('style')+";-moz-transform: scaleY(-1);-webkit-transform: scaleY(-1);transform: scaleY(-1);filter: FlipV;")});
		}
	};
	...
	window.onload=function(){
		...
		type=type.split(',');
		for(var i=-1,len=type.length;++i<len;)
			types[type[i]]();
	};
